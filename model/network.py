...
class SiameseNetV2(nn.Module):
    ...
    def __init__(self, num_dims: int = 256):
        ...
        self.embedding_net = EmbeddingNetEfficientNetB2(num_dims)
...
class BiRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        #central_index = out.size(1) // 2
        #central_element = out[:, central_index, :]

        # weights = self.get_gaussian_weights(out.size(1))
        # weights = torch.unsqueeze(weights, dim=0)  # Add batch dimension
        # weights = torch.unsqueeze(weights, dim=-1)  # Add dim dimension
        # weights = weights.to(out.device)
        # weights_out = out * weights


        # Decode the hidden state of the last time step
        #Uncomment next two lines for not sliding window approach
        out = self.fc(out)
    ...
    def get_gaussian_weights(self, sequence_length):
        middle = (sequence_length - 1) / 2
        x = torch.arange(sequence_length)
        weights = dist.Normal(middle, middle/2).log_prob(x)
        return weights
