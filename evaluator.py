...
class BlinkEvaluator(EvaluatorInterface):
    ...
    def convert_annotation_to_blinks(self, dataframe):
        i = 0
        blinks = []
        if 'gt_blink_id' not in dataframe.columns:
            return blinks
        while i < len(dataframe):
            if(dataframe.loc[i]['gt_blink_id'] != -1):
                id = dataframe.loc[i]['gt_blink_id']
                fully_closed = False
                not_visible = False
                start = dataframe.loc[i]['frameId']
                while i < len(dataframe) and dataframe.loc[i]['gt_blink_id'] == id:
                    if dataframe.loc[i]['gt_blink'] == 1:
                        fully_closed = True
                    if dataframe.loc[i]['gt_NV'] == 1:
                        not_visible = True
                    i += 1
                i -= 1
    ...
    def convert_predictions_to_blinks(self, dataframe):
        i = 0
        blinks = []
        if 'pred_blink' not in dataframe.columns:
            return blinks
        while i < len(dataframe):
            if(dataframe.loc[i]['pred_blink'] > 0):
                fully_closed = False
                not_visible = False
                start = dataframe.loc[i]['frameId']
                while i < len(dataframe) and dataframe.loc[i]['pred_blink'] > 0:
                    if dataframe.loc[i]['pred_blink'] == EYE_CLOSED:
                        fully_closed = True
                    if 'gt_NV' in dataframe.columns and dataframe.loc[i]['gt_NV'] == 1:
                        not_visible = True
                    i += 1
                i -= 1
                end = dataframe.loc[i]['frameId']
...
class BlinkDetectionEvaluator(BlinkEvaluator):
    ...
    def evaluate(self, dataframe):
        ...
        gt_blinks_left, pred_blinks_left = self.extract_blinks_per_video(left_eyes, 'LEFT')
        gt_blinks_right, pred_blinks_right = self.extract_blinks_per_video(right_eyes, 'RIGHT')
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Blink extraction time taken: {elapsed_time:.2f} seconds")
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Confusion matrix time taken: {elapsed_time:.2f} seconds")
    ...
    def extract_blinks_per_video_async(self, dataframe):
        num_videos = dataframe['video'].max()

        def process_video(i):
            video_dataframe = dataframe[dataframe['video'] == i].reset_index()
            return self.calculate_blinks_for_video(video_dataframe, i)

        with concurrent.futures.ThreadPoolExecutor() as executor:
            results = list(executor.map(process_video, range(1, int(num_videos) + 1)))

        all_gt_blinks = [result[0] for result in results]
        all_pred_blinks = [result[1] for result in results]

        return all_gt_blinks, all_pred_blinks

    def extract_blinks_per_video(self, dataframe, eye):
        if dataframe.empty:
            return [], []
        num_videos = dataframe['video'].max()
        ...
        for i in range(1, int(num_videos) + 1):
            video_dataframe = dataframe[dataframe['video'] == i].reset_index()
            gt_blinks, pred_blinks = self.calculate_blinks_for_video(video_dataframe, i, eye)
    ...
    def calculate_blinks_for_video(self, video_dataframe, video_id, eye):

        gt_blinks = self.get_gt_blinks(video_dataframe, video_id, eye)
    ...
    @cachedmethod(cache=lambda self: self._cache, key=lambda self, video_dataframe, video_id, eye: str(video_id) + eye)
    def get_gt_blinks(self, video_dataframe, video_id, eye):
...
class BlinkCompletenessDetectionEvaluator(BlinkEvaluator):
    ...
    def evaluate(self, dataframe):
        left_eyes, right_eyes = self.separate_left_right_eyes(dataframe)
        partial_gt_left_blinks, complete_gt_left_blinks, partial_pred_left_blinks, complete_pred_left_blinks = self.extract_blinks_per_video(left_eyes, 'LEFT')
        partial_gt_right_blinks, complete_gt_right_blinks, partial_pred_right_blinks, complete_pred_right_blinks = self.extract_blinks_per_video(right_eyes, 'RIGHT')
    ...
    def extract_blinks_per_video_async(self, dataframe):
        ...
        def process_video(i):
            video_dataframe = dataframe[dataframe['video'] == i].reset_index()
            return self.calculate_blinks_for_video(video_dataframe)

        with concurrent.futures.ThreadPoolExecutor() as executor:
            results = list(executor.map(process_video, range(1, int(num_videos) + 1)))

        all_partial_gt_blinks = [result[0] for result in results]
        all_complete_gt_blinks = [result[1] for result in results]
        all_partial_pred_blinks = [result[2] for result in results]
        all_complete_pred_blinks = [result[3] for result in results]
    ...
    def extract_blinks_per_video(self, dataframe, eye):
        ...
        for i in range(1, int(num_videos) + 1):
            video_dataframe = dataframe[dataframe['video'] == i].reset_index()
            partial_gt_blinks, complete_gt_blinks, partial_pred_blinks, complete_pred_blinks = self.calculate_blinks_for_video(video_dataframe, i, eye)
    ...
    def calculate_blinks_for_video(self, video_dataframe, video_id, eye):
        ...
        partial_gt_blinks, complete_gt_blinks = self.get_gt_blinks(video_dataframe, video_id, eye)
    ...
    def get_gt_blinks(self, video_dataframe, video_id, eye):
        ...
        partial_gt_blinks = self.merge_double_blinks(self.delete_non_visible_blinks(partial_gt_blinks))
        complete_gt_blinks = self.merge_double_blinks(self.delete_non_visible_blinks(complete_gt_blinks))
        return partial_gt_blinks, complete_gt_blinks
